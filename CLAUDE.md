...
**Shipped in repo:** NBP p25/p75/SD + optional 5-knot CDF; Open-Meteo GEFS/ECMWF ensemble spread columns (logging-only); **intraday observed-max truncation + HRRR shift (collector only)**; Datasette queries for triplet-vs-quint `model_prob`, edge by `forecast_lead_hours`, ensemble vs NBM spread vs |error|, and intraday Brier/truncation impact. **VM follow-up:** `git pull` and confirm `ensemble-api.open-meteo.com` + `api.open-meteo.com` are reachable from cron (if blocked, ensemble/HRRR columns stay NULL until network allows).
